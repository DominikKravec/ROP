...
export async function getDrink(drinkId){
    try {
        const drink = await databases.listDocuments(
            appwriteConfig.databaseId,
            appwriteConfig.drinkCollectionId,
            [Query.equal('$id', drinkId)]
        )

        return drink.documents[0]
    } catch (error) {
        console.log("Couldn't get drink due to: " + error)
    }
}
...
export async function getMonthLogs(userId){
    try {
        ...
        const logs = await databases.listDocuments(
            ...
            [
                Query.equal('user', userId),
                Query.greaterThanEqual('timeOfDrink', monthAgo),
                Query.limit(1000)
            ]
        )
...
export async function getLogsFromDate(userId, date){
    try {
        ...
        const logs = await databases.listDocuments(
            appwriteConfig.databaseId,
            appwriteConfig.logCollectionId,
            [Query.between('timeOfDrink', startOfDate, endOfDate)]
        )

        return logs.documents
    } catch (error) {
        console.log("Error getting logs from date: ", error);
    }
}